...
afolder = '/work/noaa/hwrf/scrub/maristiz/direct_increm_update_OBC3DF_mom6_minus_rtofs/com/2020072812/00L/'
...
def get_fld_target(afile,var_name,oklon,oklat):

    lines = [line.rstrip() for line in open(afile)]
    ijdm = idm*jdm
    npad = 4096-(ijdm%4096)
...
oklon = np.where(lon_hycom[0,:]-360 <= lon_target)[0][-1]
oklat = np.where(lat_hycom[:,0] <= lat_target)[0][-1]

####################################################################

afiles = sorted(glob.glob(afolder+'*hafs_hycom_hat10*.b'))

fld_time_series = np.empty((len(afiles)))
fld_time_series[:] = np.nan
for afile,n in enumerate(afiles):
    print(n,afile)
    fld_target = get_fld_target(afile,var_name,oklon,oklat)
    if var_name == 'srfhgt':
        fld_target = fld_target*10
    fld_time_series[n] = fld_target

fig,ax1 = plt.subplots(figsize = (8,6))
ax1.plot(np.arange(len(afiles)),fld_time_series,'.-')
