"""
  Plot SSH using SSH from 2Dvar analysis fields
...
fdlon  = mvalid.find_fileindir(pthanls, 'regional.grid_GLBb0.08.a')
fdlat  = mvalid.find_fileindir(pthanls, 'regional.grid_GLBb0.08.a')
...
#fanls  = f'seahgt_sfc_4500x3298_{rdateP}00_0000_analfld'
fanls  = f'seahgt_sfc_4500x3298_2023123100_analfld.a'
fdanls = os.path.join(pthanls,fanls)
SSH    = mncoda.read_2Dbin(fdanls, igrd, jgrd)
SSH    = np.where(SSH <= -998, np.nan, SSH)

# Plot fields:
kw = dict(levels=np.arange(-3,33+0.1,2))
plt.figure(figsize=(10,5))
plt.contourf(SSH,cmap='Spectral_r',**kw,extend='both')
