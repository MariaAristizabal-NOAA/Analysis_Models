...
print('Extracting UFLX and VFLX at surface')

#grib2file = os.path.join(conf['COMarafs']+conf['ymdh']+'/00E/', fname)
grib2file = os.path.join(conf['COMarafs']+conf['ymdh']+'/00E/', fname)
grb = grib2io.open(grib2file,mode='r')
uflx = grb.select(shortName='UFLX')[0].data
vflx = grb.select(shortName='VFLX')[0].data
mflx = np.sqrt(uflx**2 + vflx**2)
...
cflevels = np.arange(0,1.7,0.1)
cf = plt.contourf(lon_atm,lat_atm,mflx,levels=cflevels,cmap='Purples',extend='max',alpha=1,transform=transform)
cb = plt.colorbar(cf, orientation='vertical', pad=0.03, extendrect=True,shrink=0.85)

skip = 100
Q = ax.quiver(lon_atm[::skip,::skip], lat_atm[::skip,::skip], -uflx[::skip,::skip], -vflx[::skip,::skip], scale=10,linewidth=0.2, color='black',alpha=1)
ax.quiverkey(Q, 1.1, 1.1, 1, r'$1 \frac{N}{m^{2}}$', labelpos='E', coordinates='axes')
...
title = 'Momemtum Flux (N/m$^2$) \n' + conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')

ax.set_title(title, loc='center')

ax.text(0.05,0.05,'Mean = ' + str(np.round(np.nanmean(mflx),1)) + ' (N/m$^2$) ' + 'Max = ' + str(np.round(np.nanmax(mflx),1)) + ' (N/m$^2$)',transform=ax.transAxes)
