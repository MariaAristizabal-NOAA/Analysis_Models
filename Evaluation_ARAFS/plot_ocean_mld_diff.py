...
with open('plot_ocean_diff.yml', 'rt') as f:
...
fnamei =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.'+conf['fhhhi']+'.nc'
ncfile = os.path.join(conf['COMarafs'], fnamei)
nc = xr.open_dataset(ncfile)

vari = np.asarray(nc['MLD_0125'][0,:,:])

fnamef =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.'+conf['fhhhf']+'.nc'
ncfile = os.path.join(conf['COMarafs'], fnamef)
nc = xr.open_dataset(ncfile)

varf = np.asarray(nc['MLD_0125'][0,:,:])
...
cflevels = np.arange(-100,101,10)
cmap = plt.get_cmap('bwr_r')
cf = ax.contourf(lon, lat, diff, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
lb = ax.contour(lon, lat, diff, levels=[0], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
...
var_info = 'Mixed Layer Depth Deepening (MLD_0125) ($m$)'
storm_info = conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhhf'].upper()+'-'+conf['fhhhi'].upper()
