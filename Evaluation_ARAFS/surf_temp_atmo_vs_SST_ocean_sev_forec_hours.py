...
"""This script is to plot out ARAFS atmospheric surface temperature and ARAFS ocean SST"""
...
mean_diff = np.empty((len(conf['fhhhs'])))
mean_diff[:] = np.nan
std_diff = np.empty((len(conf['fhhhs'])))
std_diff[:] = np.nan
...
for f,fhhh in enumerate(conf['fhhhs']):
    ...
    #################################################################
    # Calculate differen between sst atm and sst mom6
    diff = np.ravel(tmp_sub_interp-sst_sub)
    mean_diff[f] = np.nanmean(diff)
    std_diff[f] = np.nanstd(diff)
    ...
    plt.figure(figsize=(12,8))
    cflevels = np.arange(-0.35,0.36,0.1)
...
plt.errorbar(conf['fhhhs'],mean_diff,std_diff,fmt='o-',capsize=5,ecolor='grey')
plt.plot(conf['fhhhs'],np.zeros(len(conf['fhhhs'])),color='k')
plt.ylabel('SST diff ($^oC$)')
plt.xlabel('Forecast Hour')
plt.savefig('mean_SST_diff',bbox_inches = 'tight',pad_inches = 0.1)
