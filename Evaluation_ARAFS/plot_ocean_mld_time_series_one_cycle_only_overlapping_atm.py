...
for exp in np.arange(len(conf['COMmodels'])): 
    ...
    for f,fh in enumerate(ff):
        ...
        try: 
            ...
            mld_mean[exp,f] = np.nanmean(mld_atm_domain)
            mld_max[exp,f] = np.nanmax(mld_atm_domain)
            mld_min[exp,f] = np.nanmin(mld_atm_domain)
            mld_std[exp,f] = np.nanstd(mld_atm_domain)
