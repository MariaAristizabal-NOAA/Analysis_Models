...
IVT_u1, IVT_v1, IVT1 = compute_ivt(grb,nlat, nlon)

grib2file = os.path.join(conf['COMmodels'][1]+conf['ymdh']+'/00E/', fname)
print(f'grib2file: {grib2file}')
grb = grib2io.open(grib2file,mode='r')

IVT_u2, IVT_v2, IVT2 = compute_ivt(grb,nlat, nlon)

diff = gaussian_filter(IVT2 - IVT1,20)
...
title = 'Diff IVT ($Kg m^{-1} s^{-1}$) \n' + conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')

ax.set_title(title, loc='center')

ax.text(0.1,0.1,'Mean = ' + str(np.round(np.nanmean(diff),1)) + ' ($Kg m^{-1} s^{-1}$) \n' + 'Max = ' + str(np.round(np.nanmax(diff),1)) + ' ($Kg m^{-1} s^{-1}$)' + ' Min = ' + str(np.round(np.nanmin(diff),1)) + ' ($Kg m^{-1} s^{-1}$)',transform=ax.transAxes)
