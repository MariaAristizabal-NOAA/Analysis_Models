    ...
    for c,ymdh in enumerate(conf['ymdhs']):
        ...
        for f,fh in enumerate(ff):
            ...
            try: 
                ...
                mld_atm_domain = mld * data_interp
                mld_mean[exp,c,f] = np.nanmean(mld_atm_domain)
                mld_max[exp,c,f] = np.nanmax(mld_atm_domain)
                mld_min[exp,c,f] = np.nanmin(mld_atm_domain)
                mld_std[exp,c,f] = np.nanstd(mld_atm_domain)
...
plt.xlim([0,121])
plt.xticks(np.arange(0,120,12))
plt.savefig("/ncrc/home1/Maria.Aristizabal/Figures_coupled_AR_AFS_paper/MLD_domain_mean.png", format="png", dpi=600, bbox_inches="tight", pad_inches=0.02)
