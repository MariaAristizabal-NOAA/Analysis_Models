...
if not os.path.isfile(grib2file):
    fname = conf['ymdh']+'.'+conf['stormModel'].lower()+'.'+conf['stormDomain']+'.atm.'+conf['fhhh']+'.grb2'
    grib2file = os.path.join(conf['COMarafs'], fname)
...
#ax.set_extent([xlim[0], xlim[1], ylim[0], ylim[1]], crs=transform)
ax.set_extent([xlim[0], xlim[1], ylim[0], ylim[1]])

#cf = ax.contour(lon, lat, lhf, np.arange(0,300,50),colors='grey',alpha=1,linewidths=0.5, transform=transform)
#ax.clabel(cf,cf.levels,inline=True,fmt='%1.1f',fontsize=6)

#wb = ax.barbs(lon[::skip,::skip], lat[::skip,::skip], ugrd[::skip,::skip], vgrd[::skip,::skip],length=wblength, linewidth=0.2, color='black', transform=transform)
wb = ax.quiver(lon[::skip,::skip], lat[::skip,::skip], ugrd[::skip,::skip], vgrd[::skip,::skip], color='black', transform=transform)
ax.quiverkey(wb, X=0.9, Y=1.08, U=20,label='20 m/s', labelpos='E')
