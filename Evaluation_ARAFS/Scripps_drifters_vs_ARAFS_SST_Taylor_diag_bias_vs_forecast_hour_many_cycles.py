...
def get_corresponding_model_array_from_obs_array_nc_file(timestamp_obss,lon_obss,lat_obss,var_obss,files_model,time_model_name,lon_model,lat_model,var_model_name,depth_level):
    ...
    subvar_obs = np.empty((len(files_model),1000))
    subvar_obs[:] = np.nan
    subvar_model = np.empty((len(files_model),1000))
    subvar_model[:] = np.nan
    ...
    for x,file in enumerate(files_model):
        ...
        fhour.append(int(file.split('f')[-1][0:3]))
        ...
        if len(var_obs_sort[ind]) !=0:
            subvar_obs[x,0:len(var_obs_sort[ind])] = var_obs_sort[ind]
        ...
        var[var==0] = np.nan        
        if len(var) !=0:
            subvar_model[x,0:len(var)] = var        

    fhour = np.asarray(fhour)

    return subvar_obs,subvar_model,fhour
...
corr[:] = np.nan
forec_hour = np.empty((len(folder_exps),len(cycles),41))
forec_hour[:] = np.nan
bias_forec_hour = np.empty((len(folder_exps),len(cycles),41))
bias_forec_hour[:] = np.nan
...
for i,folder in enumerate(folder_exps):
    ...
    for c,cycle in enumerate(cycles):
        ...
        subsst_obs, subsst_model, fhour = get_corresponding_model_array_from_obs_array_nc_file(timestamp_obss,lon_obss,lat_obss,var_obss,files_model,time_model_name,lon_model,lat_model,var_model_name,depth_level)
    
        #######################################################################        
        forec_hour[i,c,:] = fhour 
        for f in np.arange(len(files_model)):
            bias_forec_hour[i,c,f] = np.nanmean(subsst_model[f,:]) - np.nanmean(subsst_obs[f,:])
...
for c,cycle in enumerate(cycles):
    plt.plot(time_cycles[c],bias[0,c],'o-',color=cycles_colors[c],markersize=7,markeredgecolor='k',label=cycle)
...
# Bias plot per cycle and per forecast hour
fig,ax = plt.subplots(figsize=(11,5))
for c,cycle in enumerate(cycles):
    plt.plot(forec_hour[0,c,:],bias_forec_hour[0,c,:],'--k')
    plt.plot(forec_hour[0,c,:],bias_forec_hour[0,c,:],'o-',color=cycles_colors[c],markersize=3,markeredgecolor='k',label=cycle)
plt.legend(ncol=3, loc='upper center')
#plt.legend(loc='upper right',bbox_to_anchor=[1.1,1.1])
plt.title('SST Bias ',fontsize=18)
plt.ylabel('($^oC$)',fontsize=14)
...
#plt.ylim([-0.5,0])
plt.xlim([0,121])
ax.xaxis.set_ticks(np.arange(0,121,12))
