...
grib2file = os.path.join(conf['COMmodels'][0]+conf['ymdh']+'/00E/', fname)
grb = grib2io.open(grib2file,mode='r')
lhf1 = grb.select(shortName='LHTFL')[0].data

grib2file = os.path.join(conf['COMmodels'][1]+conf['ymdh']+'/00E/', fname)
grb = grib2io.open(grib2file,mode='r')
lhf2 = grb.select(shortName='LHTFL')[0].data

diff = gaussian_filter(lhf2 - lhf1,10) 
...
print('Plotting Latent Heat differences')
...
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, extendrect=True,shrink=0.85)
