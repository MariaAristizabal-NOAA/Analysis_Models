...
cf = plt.contourf(lon_atm,lat_atm,tmp-tmp0,levels=cflevels,cmap='turbo',extend='both',alpha=1,transform=transform)
...
ax.text(0.05,0.1,'Mean = ' + str(np.round(np.nanmean(tmp),1)) + ' ($^oC$) ' + ' Max = ' + str(np.round(np.nanmax(tmp),1)) + ' ($^oC$)'+ ' Min = ' + str(np.round(np.nanmin(tmp),1)) + ' ($^oC$)',transform=ax.transAxes)
