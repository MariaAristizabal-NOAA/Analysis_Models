...
fnam0 =  conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.'+'f000.nc'

ncfile = os.path.join(conf['COMarafs']+conf['ymdh']+'/00E/', fname) 
ncfil0 = os.path.join(conf['COMarafs']+conf['ymdh']+'/00E/', fnam0) 
if os.path.exists(ncfile):
    nc = xr.open_dataset(ncfile)
    n0 = xr.open_dataset(ncfil0)
else:
    print('file ',ncfile,' does not exist. Exiting plotting script')
    sys.exit()

var = np.asarray(nc['SST'][0,:,:])
va0 = np.asarray(n0['SST'][0,:,:])
...
cf = plt.contourf(lon_ocn+180, lat_ocn, var, levels=cflevels, cmap=cmap, extend='both',transform=transform)
plt.colorbar(cf,orientation='vertical', aspect=20,shrink=0.8,extendrect=True)
#plt.contour(lon_ocn, lat_ocn, var, levels=[15,20,26], color='black')

skip = 100
Q = ax.quiver(lon_atm[::skip,::skip]-360, lat_atm[::skip,::skip], ugrd[::skip,::skip], vgrd[::skip,::skip], scale=600,linewidth=0.2, color='black',alpha=1,transform=transform)
...
cf = plt.contourf(lon_ocn+180, lat_ocn, var-va0, levels=cflevels, cmap=cmap, extend='both',transform=transform)
plt.colorbar(cf,orientation='vertical', aspect=20,shrink=0.8,extendrect=True)

skip = 100
Q = ax.quiver(lon_atm[::skip,::skip]-360, lat_atm[::skip,::skip], ugrd[::skip,::skip], vgrd[::skip,::skip], scale=600,linewidth=0.2, color='black',alpha=0.8,transform=transform)
ax.quiverkey(Q, 1.1, 1.05, 30, r'$30 \frac{m}{s}$', labelpos='E', coordinates='axes')
...
title = 'Sea Surface Temperature Change ($^oC$) \n' + conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
...
plt.contourf(lon_atm-360,lat_atm,atm_domain,alpha=0.2,transform=transform)
