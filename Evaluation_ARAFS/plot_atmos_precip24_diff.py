...
if int(conf['fhhh'][1:]) - 24 >= 0:
    ...
    apcp1 = np.empty((len(fhhhs),lat.shape[0],lat.shape[1]))
    apcp1[:] = np.nan
    apcp2 = np.empty((len(fhhhs),lat.shape[0],lat.shape[1]))
    apcp2[:] = np.nan
    for f,fhhh in enumerate(fhhhs):
        ...
        print(grib2file)
        grb = grib2io.open(grib2file,mode='r')
        #apcp1[f,:,:] = grb.select(shortName='APCP')[0].data*0.0393701  # convert kg/m^2 to in
        apcp1[f,:,:] = grb.select(shortName='APCP')[0].data  # convert kg/m^2 to in

        grib2file = os.path.join(conf['COMmodels'][1]+conf['ymdh']+'/00E/', fname)
        print(grib2file)
        grb = grib2io.open(grib2file,mode='r')
        #apcp2[f,:,:] = grb.select(shortName='APCP')[0].data*0.0393701  # convert kg/m^2 to in
        apcp2[f,:,:] = grb.select(shortName='APCP')[0].data  # convert kg/m^2 to in

apcp1_24 = np.sum(apcp1,axis=0)
apcp2_24 = np.sum(apcp2,axis=0)
diff = gaussian_filter(apcp2_24 - apcp1_24,10)
#diff = apcp2_24 - apcp1_24
...
cf = ax.contourf(lon, lat, diff, cflevels, cmap='coolwarm', transform=transform,extend='both')
#cf = ax.contourf(lon, lat, diff, cmap='coolwarm', transform=transform,extend='both')
cb = plt.colorbar(cf, orientation='vertical', aspect=20,shrink=0.8,pad=0.03,extendrect=True,ticks=cflevels[::2])
...
plt.xlim([-125+lon_offset, -112+lon_offset])
plt.ylim([30, 50])
