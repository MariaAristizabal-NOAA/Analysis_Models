...
cf = plt.contourf(lon_atm,lat_atm,lhf,levels=cflevels,cmap='afmhot_r',extend='both',alpha=1,transform=transform)
...
ax.text(0.1,0.1,'Mean = ' + str(np.round(np.nanmean(lhf),1)) + ' (W/m$^2$) ' + 'Max = ' + str(np.round(np.nanmax(lhf),1)) + ' (W/m$^2$)',transform=ax.transAxes)
