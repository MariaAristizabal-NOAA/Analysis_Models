...
"""This script is to compare ARAFS skin temperature with ERA3"""
...
grib2file = os.path.join(COMmodel_ocean+ymdhs[0]+'/00E/', fname)
...
for ex,exp in enumerate(exp_labels):
    ...
    for y,ymdh in enumerate(ymdhs):
        ...
        for f,fhhh in enumerate(fhhhs):
            ...
            lon_sub = lon
            lat_sub = lat
            tmp_sub = tmp * data_interp

            # interpolation from fv3 to ERA5 grid
            # --- Original grid ---
            ...
            plt.plot(np.arange(-30,32),np.arange(-30,32),'-k')
            plt.legend()
            plt.title(ymdh + ' ' + fhhh)
