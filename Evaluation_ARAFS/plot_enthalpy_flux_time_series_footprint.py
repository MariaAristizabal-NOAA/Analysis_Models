...
with open('plot_atmos5.yml', 'rt') as f:
...
for exp in np.arange(len(conf['COMmodels'])): 
    ...
    for f,fh in enumerate(ff):
        ...
        fname = conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.'+conf['stormDomain']+'.atm.'+'f'+fhour+'.grb2'
        grib2file = os.path.join(conf['COMmodels'][exp]+conf['ymdh']+'/'+conf['stormID'], fname)
        if os.path.exists(grib2file):
            grib2file = grib2file
        else:
            fname = conf['ymdh']+'.'+conf['stormModel'].lower()+'.'+conf['stormDomain']+'.atm.'+'f'+fhour+'.grb2'        
            grib2file = os.path.join(conf['COMmodels'][exp]+conf['ymdh']+'/'+conf['stormID'], fname)
...
for exp in np.arange(len(conf['COMmodels'])):
    ...
    plt.plot(ff,lhfl_max[exp,:],'.-',color=conf['exp_colors'][exp],markeredgecolor='k',markersize=7)
ax.legend()
ax.tick_params(which='major', width=2)
ax.tick_params(which='major', length=7)
ax.tick_params(which='minor', length=4, color='k')
...
for exp in np.arange(len(conf['COMmodels'])):
    ...
    plt.plot(ff,totfl_max[exp,:],'.-',color=conf['exp_colors'][exp],markeredgecolor='k',markersize=7)
