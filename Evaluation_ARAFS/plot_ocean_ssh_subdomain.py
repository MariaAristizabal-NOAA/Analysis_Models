...
oceanf = glob.glob(os.path.join(conf['COMarafs']+conf['ymdh']+'/00E/','*f006.nc'))[0].split('/')[-1].split('.')
...
if ocean == 'mom6':
    ...
    lon = np.asarray(nc.geolon)
    lat = np.asarray(nc.geolat)
...
cflevels = np.arange(-0.5,0.51,0.05)
...
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.7, extendrect=True, ticks=cflevels[::4])
cb.ax.tick_params(labelsize=8)

ax.set_extent([xlim[0], xlim[1], ylim[0], ylim[1]], crs=ccrs.PlateCarree())
#ax.set_extent([-210, -70, -20, 70], crs=ccrs.PlateCarree())
