...
if ocean == 'mom6':
    fname0 =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.'+'f000.nc'
    fname =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.'+conf['fhhh']+'.nc'

if ocean == 'hycom':
    fname0 =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.hycom.3z.'+'f000.nc'
    fname =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.hycom.3z.'+conf['fhhh']+'.nc'

ncfile0 = os.path.join(conf['COMhafs'], fname0)
nc0 = xr.open_dataset(ncfile0)
ncfile = os.path.join(conf['COMhafs'], fname)
nc = xr.open_dataset(ncfile)

if ocean == 'mom6':
    sstt0 = np.asarray(nc0['SST'][0,:,:])
    sstt = np.asarray(nc['SST'][0,:,:])
    lon_ocean = np.asarray(nc.xh)
    lat_ocean = np.asarray(nc.yh)

if ocean == 'hycom':
    sstt0 = np.asarray(nc0['temperature'][0,0,:,:])
    sstt = np.asarray(nc['temperature'][0,0,:,:])
    lon_ocean = np.asarray(nc.Longitude)
    lat_ocean = np.asarray(nc.Latitude)

print('raw lonlat limit: ', np.min(lon_ocean), np.max(lon_ocean), np.min(lat_ocean), np.max(lat_ocean))

#================================================================
# Constrain lon limits between -180 and 180 so it does not conflict with the cartopy projection PlateCarree
lon_ocean[lon_ocean>180] = lon_ocean[lon_ocean>180] - 360
lon_ocean[lon_ocean<-180] = lon_ocean[lon_ocean<-180] + 360
sort_lon = np.argsort(lon_ocean)
lon_ocean = lon_ocean[sort_lon]

# sort var according to the new longitude
sstt0 = sstt0[:,sort_lon]
sstt = sstt[:,sort_lon]

Rkm=500    # search radius [km]
lns,lts = np.meshgrid(lon_ocean,lat_ocean)
...
sst = sstt*dumb
dsst = np.asarray(sstt - sstt0)*dumb
...
if conf['stormDomain'] == 'storm':
    ...
    cbshrink = 1.0
    lonmin = lon[int(nlat/2), int(nlon/2)]-4
    lonmax = lon[int(nlat/2), int(nlon/2)]+4
    lonint = 2.0
    latmin = lat[int(nlat/2), int(nlon/2)]-4
    latmax = lat[int(nlat/2), int(nlon/2)]+4
    latint = 2.0
    skip = 20
    wblength = 4.5
else:
...
#cf = ax.contour(lon, lat, lhf, np.arange(0,1201,200),colors='grey',alpha=1,linewidths=0.5, transform=transform)
#ax.clabel(cf,cf.levels,inline=True,fmt='%1.1f',fontsize=6)

cf = ax.contour(lon_ocean, lat_ocean, dsst, [-2], colors='blue', alpha=1, linewidths=1, transform=ccrs.PlateCarree())
ax.clabel(cf,cf.levels,inline=True,fmt='%1.1f',fontsize=6)

cf = ax.contour(lon_ocean, lat_ocean, dsst, [-1], colors='deepskyblue', alpha=1, linewidths=1, transform=ccrs.PlateCarree())
ax.clabel(cf,cf.levels,inline=True,fmt='%1.1f',fontsize=6)
...
lhfff = lhff[oklat]
mnmx = "(max)="+"(%6.1f)"%np.nanmax(lhfff)
ax.text(lonmin+0.1,latmin+0.1,mnmx,fontsize=8,color='k',fontweight='bold',bbox=dict(boxstyle="round",color='w',alpha=0.5))
