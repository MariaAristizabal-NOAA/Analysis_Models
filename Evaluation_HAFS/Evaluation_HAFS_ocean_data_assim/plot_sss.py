...
if conf['trackon']=='yes':
    ...
    adeck_file = os.path.join(conf['COMhafs1'][:-10]+'hafstrak/hafsv2_hnod_hnod/',adeck_name)
...
if ocean == 'mom6':
    var1 = np.asarray(nc1['SSS'][0,:,:])
    var2 = np.asarray(nc2['SSS'][0,:,:])
...
lb = ax.contour(lon, lat, var1, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
...
var_info = 'Sea Surface Salinity (psu) ' + conf['exp1_name']
...
lb = ax.contour(lon, lat, var2, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Salinity (psu) ' + conf['exp2_name']
storm_info = conf['stormName']+conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
...
lb = ax.contour(lon, lat, var2-var1, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Salinity (psu) ' + conf['exp2_name'] + '-' + conf['exp1_name']
storm_info = conf['stormName']+conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
