...
if lat_adeck[nhour] < (latmax+5.0):
    ...
    ax.contour(lon, lat, var, cflevels, colors='grey',alpha=0.5, linewidths=1.0, transform=ccrs.PlateCarree())
    ...
    lb = ax.contour(lon, lat, dvar, cflevels, colors='grey', alpha=0.5, linewidths=1.0, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
