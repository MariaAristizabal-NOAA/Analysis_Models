...
for code in [1301700.]:
    ...
    for i,folder in enumerate(folder_exps):
        ...
        if exp_labels[i].split('_')[0] == 'epbl' or exp_labels[i].split('_')[0] == 'kpp' or exp_labels[i] == 'HFSAv1p1' or exp_labels[i] == 'HFSAv2a_baseline' or exp_labels[i] == 'HFSAv2a_baseline_latest' or exp_names[i] == 'HKPP' or exp_names[i] == 'HKP2':
    ...
    plt.title('Track Forecast ' + storm_num + ' cycle '+ cycle,fontsize=18)
    plt.axis('scaled')
    plt.xlim([np.nanmin(lond)-2,np.nanmax(lond)+2])
    plt.ylim([np.nanmin(latd)-2,np.nanmax(latd)+2])
