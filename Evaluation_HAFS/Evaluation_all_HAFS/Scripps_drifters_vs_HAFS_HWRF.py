...
target_temp = np.empty((len(folder_exps),22))
target_temp[:] = np.nan
target_salt = np.empty((len(folder_exps),22))
target_salt[:] = np.nan
...
target_timed_ocean = [[0 for i in range(22)] for j in range(len(folder_exps))]
...
for code in [4102649.0,4102651.0,4102654.0]:
    ...
    for i,folder in enumerate(folder_exps):
        ...
        if exp_labels[i] == 'HFSA' or exp_labels[i] == 'HFSB':
            ...
            target_timed_ocean[i], target_temp[i,:] = get_var_from_model_following_trajectory(files_model,'temperature',time_name,lon_name,lat_name,lon_obs,lat_obs,timestamp_obs,**kwargs)
            _, target_salt[i,:] = get_var_from_model_following_trajectory(files_model,'salinity',time_name,lon_name,lat_name,lon_obs,lat_obs,timestamp_obs,**kwargs)
            ...
            target_timed_atm[i], target_press_surf[i,:] = get_var_from_model_following_trajectory(files_model,'PRES_P0_L1_GLL0',time_name,lon_name,lat_name,lon_obs,lat_obs,timestamp_obs)
        ...
        if exp_labels[i] == 'HWRF':
            ...
            target_timed_ocean[i], target_temp[i,0:len(files_model)] = get_var_from_model_following_trajectory(files_model,'t',time_name,lon_name,lat_name,lon_obs,lat_obs,timestamp_obs,**kwargs)
    ...
    for i in np.arange(len(exp_names)):
        plt.plot(target_timed_atm[i],target_press_surf[i,:]/100,'o-',color=exp_colors[i],markeredgecolor='k',label=exp_labels[i],markersize=7)
    plt.legend()
...
for code in [4102649.0,4102651.0,4102654.0]:
    ...
    for i in np.arange(len(exp_names)):
        plt.plot(lon_forec_track[i,::2], lat_forec_track[i,::2],'o-',color=exp_colors[i],markeredgecolor='k',label=exp_labels[i],markersize=7)
    plt.plot(lond, latd,'.',color='blue',label='Drifter id ' + str(code))
    plt.legend(loc='upper right',bbox_to_anchor=[1.3,0.8])
    #plt.legend(loc='upper right')
    plt.title('Track Forecast ' + storm_num + ' cycle '+ cycle,fontsize=18)
    plt.axis('scaled')
