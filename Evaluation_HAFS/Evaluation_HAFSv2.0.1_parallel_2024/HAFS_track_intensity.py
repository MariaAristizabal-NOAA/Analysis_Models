...
exp_names = ['HFSAv2a_baseline_latest','HAFSv2.0.1A','HAFSv2.0.1B']
exp_labels = ['HFSAv2a_baseline','HAFSv2.0.1A','HAFSv2.0.1B']
exp_colors = ['cyan','orange','dodgerblue']
'''
...
exp_names = ['HAFSv2p0p1a_2024rt','HAFSv2p0p1a_2024rt_BBL_EFFIC']
exp_labels = ['HFXA','HFXA_BBL_EFFIC']
exp_colors = ['cadetblue','lightcoral']
...
#plt.legend()
plt.title(storm_num + ' cycle '+ cycle,fontsize=18)
plt.axis('scaled')
...
plt.contour(bath_lon,bath_lat,bath_elev,[0],colors='k')
for i in np.arange(len(exp_names)):
    plt.plot(lon_forec_track[i,::2], lat_forec_track[i,::2],'-',color=exp_colors[i],label=exp_labels[i])
plt.plot(lon_best_track[okt], lat_best_track[okt],'-',color='k',label='Best Track')
#plt.legend(loc='upper right')
plt.legend(loc='upper right',bbox_to_anchor=[1.4,1.0])
#plt.legend()
plt.title(storm_num + ' cycle '+ cycle,fontsize=18)
