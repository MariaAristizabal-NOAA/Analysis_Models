...
for exp in np.arange(len(conf['COMhafs'])): 
    plt.plot(fhour.astype('int'),sst_mean[exp,:],'o-',color=conf['exp_colors'][exp],label=conf['exp_labels'][exp],markeredgecolor='k',markersize=7)
    ax.fill_between(fhour.astype('int'),sst_min[exp,:],sst_max[exp,:],color=conf['exp_colors'][exp],alpha=0.1)
    plt.plot(fhour.astype('int'),sst_max[exp,:],'.-',color=conf['exp_colors'][exp],markeredgecolor='k',markersize=7)
