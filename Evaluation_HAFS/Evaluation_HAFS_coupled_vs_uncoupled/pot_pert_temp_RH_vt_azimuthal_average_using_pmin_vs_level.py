...
for h,hh in enumerate(fhours):
    ...
    w_mean_dr[:] = np.nan
    lat_min_gh = np.empty((len(exp_names),nvert_lev))
    lat_min_gh[:] = np.nan
    lon_min_gh = np.empty((len(exp_names),nvert_lev))
    lon_min_gh[:] = np.nan
    ...
    for f,ff in enumerate(exp_folders):
        ...
        for z in np.arange(len(press_hafs_fv3)):
            ...
            geop_height = np.asarray(hafs_fv3['HGT_P0_L100_GLL0'][z,oklat,oklon])
            # location of local minimum geopotential height
            oklat_min_gh = np.where(geop_height==np.nanmin(geop_height))[1][0]
            oklon_min_gh = np.where(geop_height==np.nanmin(geop_height))[0][0]
            lat_min_gh[f,z] = lat_hafs_fv3[oklat][oklat_min_gh]
            lon_min_gh[f,z] = lon_hafs_fv3[oklon][oklon_min_gh]
 
            plt.figure()
            plt.contourf(lon_hafs_fv3[oklon],lat_hafs_fv3[oklat],geop_height)
            plt.plot(lon_min_gh[f,z],lat_min_gh[f,z],'*r')
            plt.colorbar()
