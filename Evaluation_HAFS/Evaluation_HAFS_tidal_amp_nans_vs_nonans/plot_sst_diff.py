...
oceanf1 = glob.glob(os.path.join(conf['COMhafs1'],'*f006.nc'))[0].split('/')[-1].split('.')

ocean = [f for f in oceanf1 if f == 'hycom' or f == 'mom6'][0]
...
#cflevels = np.arange(-0.1,0.11,0.01)
cflevels = np.arange(-0.01,0.011,0.001)
#cflevels = np.arange(-0.5,0.51,0.1)
#cflevels = np.arange(-1,1.1,0.1)
cmap = plt.get_cmap('seismic')
cf = ax.contourf(lon, lat, diff_var, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
#cf = ax.contourf(lon, lat, diff_var, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
...
##########################################################
plt.figure()
plt.hist(np.ravel(diff_var),bins=100)
