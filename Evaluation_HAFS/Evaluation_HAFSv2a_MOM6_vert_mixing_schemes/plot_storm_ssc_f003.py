...
oceanf = glob.glob(os.path.join(conf['COMhafs'],'*f003.nc'))[0].split('/')[-1].split('.')
...
if ocean == 'mom6':
    ...
    interpolator = RegularGridInterpolator((lat,lonq),ssu,bounds_error=False, fill_value=None)
    ssu_interp = interpolator((lttq,lnnh))

    varr = np.sqrt(ssu_interp**2 + ssv**2)
...
if len(lon_adeck[okfhour])!=0 and len(lat_adeck[okfhour])!=0:
    if lat_adeck[nhour] < (latmax+5.0):
        ...
        cflevels = np.arange(0,2.5,0.2)
        #cmap = plt.get_cmap('turbo')
        cmap = plt.get_cmap('rainbow')
        cf = ax.contourf(lon, latq, var, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
        ax.contour(lon, latq, var, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
        ...
        var_info = 'Sea Surface Velocity ($m/s$)'
