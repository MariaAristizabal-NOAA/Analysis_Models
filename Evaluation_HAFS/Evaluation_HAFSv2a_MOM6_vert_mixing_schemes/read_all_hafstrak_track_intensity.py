...
for i,folder in enumerate(folder_exps):
    ...
    if exp_names[i] == 'HWRF':
        file_tracks = np.sort(glob.glob(os.path.join(folder,storm_id+'*.trak.hwrf.atcfunix')))
    if exp_labels[i] == 'HMON':
        file_tracks = np.sort(glob.glob(os.path.join(folder,storm_id+'*.trak.hmon.atcfunix')))
...
for i,folder in enumerate(folder_exps):
    ...
    for c,cycle in enumerate(all_cycles):
        ...
        if exp_names[i] == 'HWRF':
            file_track = glob.glob(os.path.join(folder,storm_id+'*.trak.hwrf.atcfunix'))[0]
        if exp_labels[i] == 'HMON':
            file_track = glob.glob(os.path.join(folder,storm_id+'*.trak.hmon.atcfunix'))[0]
...
#for c,cycle in enumerate(all_cycles[13:21]): 
for c,cycle in enumerate(['2023090700','2023090712']): 
    #%% Time window
    date_ini = cycle[0:4]+'/'+cycle[4:6]+'/'+cycle[6:8]+'/'+cycle[8:]+'/00/00'
    tini = datetime.strptime(date_ini,'%Y/%m/%d/%H/%M/%S')
    tend = tini + timedelta(hours=126)
    ...
    for f,folder in enumerate(folder_exps):
        plt.plot(lon_forec_track[f,cc,::2], lat_forec_track[f,cc,::2],'o-',color=exp_colors[f],markeredgecolor='k',label=exp_labels[f],markersize=7)
...
#for c,cycle in enumerate(all_cycles[13]): 
#for c,cycle in enumerate(['2023093000','2023093006','2023093012','2023093018','2023100100','2023100106']): 
for c,cycle in enumerate(['2023090700','2023090712']): 
    #%% Time window
    date_ini = cycle[0:4]+'/'+cycle[4:6]+'/'+cycle[6:8]+'/'+cycle[8:]+'/00/00'
    tini = datetime.strptime(date_ini,'%Y/%m/%d/%H/%M/%S')
