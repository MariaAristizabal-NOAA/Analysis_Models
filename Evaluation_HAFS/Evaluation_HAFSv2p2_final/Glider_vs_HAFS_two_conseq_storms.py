...
hafs_ab = ['hfsa','hfsa','hfsb','hfsb']
ocean = ['mom6','mom6','hycom','mom6']
scratch_folder = '/scratch3/NCEPDEV/hwrf/noscrub/Maria.Aristizabal/'

url_glider = '/scratch3/NCEPDEV/hwrf/noscrub/Maria.Aristizabal/Data/Gliders/2024/usf-jaialai-20240920T0000.nc' 
...
for i in np.arange(len(exp_names)):
    folder_exps.append(scratch_folder + exp_names[i])
...
for c,cyc in enumerate(cycles):
    ...
    for i in np.arange(len(exp_names)):
        ...
        if c==0 and i==0:
            ...
            plt.plot(lon_best_track[c], lat_best_track[c],'o-',color='k')
plt.plot(long[0,:], latg[0,:],'.-',color='red',label='Glider Track')
