...
#================================================================
# Get file best track
best_track_file = conf['folder_best_track'] + '/b' + conf['stormBasin'].lower() + conf['stormID'][:-1] + conf['ymdh'][0:4] + '.dat'
print(best_track_file)
...
if len(str(conf['validTime'].hour)) == 1:
    valid_time = str(conf['validTime'].year) + str(conf['validTime'].month) + str(conf['validTime'].day) + '0' + str(conf['validTime'].hour)
else:
    valid_time = str(conf['validTime'].year) + str(conf['validTime'].month) + str(conf['validTime'].day) + str(conf['validTime'].hour)
okfhour_best = t_best_track == valid_time
...
if len(lon_adeck[okfhour])!=0 and len(lat_adeck[okfhour])!=0:
    if lat_adeck[nhour] < (latmax+5.0):
        ...
        if conf['trackon']=='yes':
            ...
            ax.plot(lon_best_track,lat_best_track,'-*k',markersize=6,alpha=0.4,transform=ccrs.PlateCarree(central_longitude=0))
            ...
            ax.plot(lon_best_track[okfhour_best],lat_best_track[okfhour_best],'ok',markersize=10,alpha=0.6,markerfacecolor='None',transform=ccrs.PlateCarree(central_longitude=0))
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                ...
                ax.set_extent([lon_adeck[nhour]-10.5,lon_adeck[nhour]+10.5,lat_adeck[nhour]-10.5,lat_adeck[nhour]+10.5],crs=ccrs.PlateCarree())
            else:
