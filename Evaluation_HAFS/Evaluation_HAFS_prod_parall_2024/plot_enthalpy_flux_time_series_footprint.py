...
shfl_sum = np.empty((len(conf['COMhafs']),len(ff)))
shfl_sum[:] = np.nan
...
totfl_min[:] = np.nan
tot_power = np.empty((len(conf['COMhafs']),len(ff)))
tot_power[:] = np.nan
...
for exp in np.arange(len(conf['COMhafs'])): 
    ...
    for f,fh in enumerate(ff):
        ...
        if f < len(fhour):
            ...
            delta_x = np.diff(lon,axis=1)
            delta_y = np.diff(lat,axis=0)
            area = delta_x[0:-1,:] * delta_y[:,0:-1] * np.cos(lat[0:-1,0:-1]*np.pi/180) * (111195)**2
            tot_power[exp,f] = np.nansum(totfl[0:-1,0:-1] * area)

fig,ax = plt.subplots(figsize = (9,6))
for exp in np.arange(len(conf['COMhafs'])): 
    plt.plot(np.arange(0,127,3),shfl_mean[exp,:],'o-',color=conf['exp_colors'][exp],label=conf['exp_labels'][exp],markeredgecolor='k',markersize=7)
    ax.fill_between(np.arange(0,127,3),shfl_min[exp,:],shfl_max[exp,:],color=conf['exp_colors'][exp],alpha=0.1)
    plt.plot(np.arange(0,127,3),shfl_max[exp,:],'.-',color=conf['exp_colors'][exp],markeredgecolor='k',markersize=7)
...
fig,ax = plt.subplots(figsize = (9,6))
for exp in np.arange(len(conf['COMhafs'])):
    plt.plot(np.arange(0,127,3),lhfl_mean[exp,:],'o-',color=conf['exp_colors'][exp],label=conf['exp_labels'][exp],markeredgecolor='k',markersize=7)
    ax.fill_between(np.arange(0,127,3),lhfl_min[exp,:],lhfl_max[exp,:],color=conf['exp_colors'][exp],alpha=0.1)
    plt.plot(np.arange(0,127,3),lhfl_max[exp,:],'.-',color=conf['exp_colors'][exp],markeredgecolor='k',markersize=7)
...
fig,ax = plt.subplots(figsize = (9,6))
for exp in np.arange(len(conf['COMhafs'])):
    plt.plot(np.arange(0,127,3),totfl_mean[exp,:],'o-',color=conf['exp_colors'][exp],label=conf['exp_labels'][exp],markeredgecolor='k',markersize=7)
    ax.fill_between(np.arange(0,127,3),totfl_min[exp,:],totfl_max[exp,:],color=conf['exp_colors'][exp],alpha=0.1)
    plt.plot(np.arange(0,127,3),totfl_max[exp,:],'.-',color=conf['exp_colors'][exp],markeredgecolor='k',markersize=7)
...
fig,ax = plt.subplots(figsize = (9,6))
for exp in np.arange(len(conf['COMhafs'])):
    tot_energy = np.nansum(tot_power[exp,:]*3*3600)
    plt.plot(np.arange(0,127,3),tot_power[exp,:],'o-',color=conf['exp_colors'][exp],label=conf['exp_labels'][exp]+' Total Energy='+format(tot_energy,'0.3e')+ 'J',markeredgecolor='k',markersize=7)
...
plt.title('Power Footprint (200 km around storm eye))',fontsize=14)
plt.xlim([0,126])
plt.xticks(np.arange(0,126,12))
...
for exp in np.arange(len(conf['COMhafs'])):
    tot_energy = np.nansum(tot_power[exp,:]*3*3600)
    accum_energy = np.cumsum(tot_power[exp,:]*3*3600)
    plt.plot(np.arange(0,127,3),accum_energy,'o-',color=conf['exp_colors'][exp],label=conf['exp_labels'][exp]+' Total Energy='+format(tot_energy,'0.3e')+ 'J',markeredgecolor='k',markersize=7)
