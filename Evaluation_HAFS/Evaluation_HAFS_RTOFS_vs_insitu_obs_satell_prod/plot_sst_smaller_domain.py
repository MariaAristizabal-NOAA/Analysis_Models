...
if ocean == 'hycom':
    var1 = np.asarray(nc1['temperature'][0,0,:,:])
    var2 = np.asarray(nc2['temperature'][0,0,:,:])
    lon = np.asarray(nc1.Longitude)
    lat = np.asarray(nc1.Latitude)
...
cflevels = np.arange(25, 32.2, 0.4)
...
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=1.0, extendrect=True, ticks=cflevels[::2])
...
var_info = 'Sea Surface Temperature (${^o}$C) ' + conf['exp1_name']
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Temperature (${^o}$C) ' + conf['exp2_name']
storm_info = conf['stormName']+conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
...
#lb = ax.contour(lon, lat, var_hoda-var_hnod, levels=[0], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=1.0, extendrect=True, ticks=cflevels[::2])
cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Temperature (${^o}$C) ' + conf['exp2_name'] + '-' + conf['exp1_name']
storm_info = conf['stormName']+conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
