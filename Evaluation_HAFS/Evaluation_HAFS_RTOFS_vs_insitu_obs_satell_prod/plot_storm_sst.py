...
if conf['trackon']=='yes':
    ...
    adeck_file = os.path.join(conf['COMhafs1'],adeck_name)
...
oceanf = glob.glob(os.path.join(conf['COMhafs1'],'*f000.nc'))[0].split('/')[-1].split('.')
...
ncfile000_1 = os.path.join(conf['COMhafs1'], fname000)
nc000_1 = xr.open_dataset(ncfile000_1)
ncfile1 = os.path.join(conf['COMhafs1'], fname) 
nc1 = xr.open_dataset(ncfile1)

ncfile000_2 = os.path.join(conf['COMhafs2'], fname000)
nc000_2 = xr.open_dataset(ncfile000_2)
ncfile2 = os.path.join(conf['COMhafs2'], fname) 
nc2 = xr.open_dataset(ncfile2)

if ocean == 'mom6':
    varr000_1 = np.asarray(nc000_1['SST'][0,:,:])
    varr1 = np.asarray(nc1['SST'][0,:,:])
    varr000_2 = np.asarray(nc000_2['SST'][0,:,:])
...
varr000_1 = varr000_1[:,sort_lon]
varr1 = varr1[:,sort_lon]
varr000_2 = varr000_2[:,sort_lon]
...
if lat_adeck[nhour] < (latmax+5.0):
    ...
    var1 = varr1*dumb
    dvar1 = np.asarray(varr1 - varr000_1)*dumb
    var2 = varr2*dumb
    dvar2 = np.asarray(varr2 - varr000_2)*dumb
    var12 = var2 - var1
    ...
    cf = ax.contourf(lon, lat, var1, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.contour(lon, lat, var1, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    ...
    var_info = 'Sea Surface Temperature ($^oC$) ' + conf['exp1_name']
    ...
    cf = ax.contourf(lon, lat, dvar1, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    lb = ax.contour(lon, lat, dvar1, cflevels, colors='grey', alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature Change ($^oC$) ' + conf['exp1_name']
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    cflevels = np.linspace(19, 32, 27)
    cmap = plt.get_cmap('turbo')
    cf = ax.contourf(lon, lat, var2, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.contour(lon, lat, var2, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature ($^oC$) ' + conf['exp2_name']
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    cflevels = np.linspace(-4, 4, 17)
    cmap = plt.get_cmap('RdBu_r')
    cf = ax.contourf(lon, lat, dvar2, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    lb = ax.contour(lon, lat, dvar2, cflevels, colors='grey', alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature Change ($^oC$) ' + conf['exp2_name']
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    cflevels = np.linspace(-4, 4, 17)
    cmap = plt.get_cmap('RdBu_r')
    cf = ax.contourf(lon, lat, var12, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.contour(lon, lat, var12, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    if conf['trackon']=='yes':
        ...
        if nhour <= len(fhour):
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                mnmx="(min,max)="+"(%6.1f"%np.nanmin(var12)+","+"%6.1f)"%np.nanmax(var12)
                plt.text(lon_adeck[nhour]-2.15-central_longitude,lat_adeck[nhour]-4.75,mnmx,fontsize=8,color='DarkOliveGreen',fontweight='bold',bbox=dict(boxstyle="round",color='w',alpha=0.5))
                ax.set_extent([lon_adeck[nhour]-5.5,lon_adeck[nhour]+5.5,lat_adeck[nhour]-5,lat_adeck[nhour]+5],crs=ccrs.PlateCarree())
            else:
                print('Longitude track limits are out of the ocean domain')
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature ($^oC$) ' + conf['exp2_name'] + '-' + conf['exp1_name']
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
