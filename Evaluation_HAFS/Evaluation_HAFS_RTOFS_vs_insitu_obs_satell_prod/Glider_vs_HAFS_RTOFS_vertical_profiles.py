...
for fg,file_glider in enumerate([files_gliders[1],files_gliders[21],files_gliders[25]]):
    print(file_glider)
    ...
    if len(timee) != 0:
        ...
        plt.plot(longg, latgg,'.-',color='orange',label=dataset_id[fg].split('-')[0])
        plt.legend(loc='upper right',bbox_to_anchor=[1.3,1.0])
        plt.title('Track Forecast ' + storm_num + ' cycle '+ cycle,fontsize=18)
        plt.axis('scaled')
        if np.isfinite(np.nanmin(longg)):
            plt.xlim([np.nanmin(longg)-4,np.nanmax(longg)+4])
            plt.ylim([np.nanmin(latgg)-4,np.nanmax(latgg)+4])
        ...
        for i,folder in enumerate(folder_exps):
            ...
            plt.plot(tempg_gridded[:,0],-depthg_gridded,'.-b',label=dataset_id[fg])
            plt.plot(target_temp_model_oc[:,0],-depth,'.-',color=exp_colors[i],label=exp_labels[i],alpha=0.5)
            plt.plot(tempg_gridded,-depthg_gridded,'.-b')
            plt.plot(target_temp_model_oc,-depth,'.-',color=exp_colors[i],alpha=0.3)
            plt.legend()
            plt.ylim([-100,0])
            plt.ylabel('Depth (m)')
