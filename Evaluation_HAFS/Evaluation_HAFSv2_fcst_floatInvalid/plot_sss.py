...
if ocean == 'mom6':
    var_hnod = np.asarray(nc_hnod['SSS'][0,:,:])
    var_hoda = np.asarray(nc_hoda['SSS'][0,:,:])
    lon = np.asarray(nc_hnod.xh)
    lat = np.asarray(nc_hnod.yh)
...
var_hnod = var_hnod[:,sort_lon]
var_hoda = var_hoda[:,sort_lon]
...
lb = ax.contour(lon, lat, var_hnod, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
...
var_info = 'Sea Surface Salinity (psu) HNOD'
...
lb = ax.contour(lon, lat, var_hoda, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Salinity (psu) HODA'
storm_info = conf['stormName']+conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
...
cf = ax.contourf(lon, lat, var_hoda-var_hnod, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
lb = ax.contour(lon, lat, var_hoda-var_hnod, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Salinity (psu) HODA-HNOD'
storm_info = conf['stormName']+conf['stormID']
title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
