...
file_hafs1 = conf['File_hafs1']
file_hafs2 = conf['File_hafs2']
...
nc1 = xr.open_dataset(file_hafs1)
nc2 = xr.open_dataset(file_hafs2)

var1 = np.asarray(nc1['SST'][0,:,:])
var2 = np.asarray(nc2['SST'][0,:,:])
...
lb = ax.contour(lon, lat, var1, levels=[26], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
...
var_info = 'Sea Surface Temperature (${^o}$C) Original ' + file_hafs1.split('/')[-1]
...
lb = ax.contour(lon, lat, var2, levels=[26], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Temperature (${^o}$C) New` ' + file_hafs1.split('/')[-1]
#storm_info = conf['stormName']+conf['stormID']
#title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
title_left = """{0}\n{1}""".format(model_info,var_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
...
#cflevels = np.arange(-10**-4, 10.1**-4, 10**-5)
cflevels = np.arange(-0.001, 0.00101, 0.0001)
cmap = plt.get_cmap('seismic')
cf = ax.contourf(lon, lat, var1-var2, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
#ax.clabel(lb, lb.levels, inline=True,fmt='%1.0f', fontsize=6,colors='grey')
#cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels)
cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True)
#cb.ax.tick_params(labelsize=8)
...
model_info = os.environ.get('TITLEgraph','').strip()
var_info = 'Sea Surface Temperature (${^o}$C) Orig - New ' + file_hafs1.split('/')[-1]
#storm_info = conf['stormName']+conf['stormID']
#title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
title_left = """{0}\n{1}""".format(model_info,var_info)
ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
...
footer = os.environ.get('FOOTERgraph','Experimental HAFS Product').strip()
ax.text(1.0,-0.1, footer, fontsize=8, va="top", ha="right", transform=ax.transAxes)
minmax = '[' + str(np.round(np.nanmin(var1-var2),3)) + ', ' + str(np.round(np.nanmax(var1-var2),3)) + ']'
ax.text(0.2,-0.2, minmax, fontsize=14, va="top", ha="right", transform=ax.transAxes)

ybad1 = np.where(var1-var2==np.nanmin(var1-var2))[0]
xbad1 = np.where(var1-var2==np.nanmin(var1-var2))[1]
ybad2 = np.where(var1-var2==np.nanmax(var1-var2))[0]
xbad2 = np.where(var1-var2==np.nanmax(var1-var2))[1]

ax.plot(lon[xbad1],lat[ybad1],'*g',transform=ccrs.PlateCarree())
ax.plot(lon[xbad2],lat[ybad2],'*',color='orange',transform=ccrs.PlateCarree())
...
plt.figure()
plt.plot(var1-var2,'.')
plt.ylabel('SST orig - SST new')

plt.figure()
plt.hist(np.ravel(var1-var2),bins=100)
