...
if conf['trackon']=='yes':
    ...
    adeck_file = os.path.join(conf['COMhafs'][:-10]+'hafstrak/hafsv2_hnod_hnod/',adeck_name)
...
oceanf = glob.glob(os.path.join(conf['COMhafs'],'*f000.nc'))[0].split('/')[-1].split('.')

ocean = [f for f in oceanf if f == 'hycom' or f == 'mom6'][0]

if ocean == 'mom6':
    fname000 =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.2D.'+'f000.nc' 
    fname =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.mom6.2D.'+conf['fhhh']+'.nc' 
...
ncfile000_hnod = os.path.join(conf['COMhafs'], fname000)
nc000_hnod = xr.open_dataset(ncfile000_hnod)
ncfile_hnod = os.path.join(conf['COMhafs'], fname) 
nc_hnod = xr.open_dataset(ncfile_hnod)

ncfile000_hoda = os.path.join(conf['COMhafs_hoda'], fname000)
nc000_hoda = xr.open_dataset(ncfile000_hoda)
ncfile_hoda = os.path.join(conf['COMhafs_hoda'], fname) 
nc_hoda = xr.open_dataset(ncfile_hoda)

if ocean == 'mom6':
    varr000_hnod = np.asarray(nc000_hnod['SST'][0,:,:])
    varr_hnod = np.asarray(nc_hnod['SST'][0,:,:])
    varr000_hoda = np.asarray(nc000_hoda['SST'][0,:,:])
    varr_hoda = np.asarray(nc_hoda['SST'][0,:,:])
    lon = np.asarray(nc_hoda.xh)
    lat = np.asarray(nc_hoda.yh)
...
varr000_hnod = varr000_hnod[:,sort_lon]
varr_hnod = varr_hnod[:,sort_lon]
varr000_hoda = varr000_hoda[:,sort_lon]
varr_hoda = varr_hoda[:,sort_lon]
...
if lat_adeck[nhour] < (latmax+5.0):
    ...
    var_hnod = varr_hnod*dumb
    dvar_hnod = np.asarray(varr_hnod - varr000_hnod)*dumb
    var_hoda = varr_hoda*dumb
    dvar_hoda = np.asarray(varr_hoda - varr000_hoda)*dumb
    var_hoda_hnod = var_hoda - var_hnod
    ...
    cf = ax.contourf(lon, lat, var_hnod, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.contour(lon, lat, var_hnod, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    ...
    if conf['trackon']=='yes':
        ...
        if nhour <= len(fhour):
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                mnmx="(min,max)="+"(%6.1f"%np.nanmin(var_hnod)+","+"%6.1f)"%np.nanmax(var_hnod)
    ...
    var_info = 'Sea Surface Temperature ($^oC$) HNOD'
    ...
    cf = ax.contourf(lon, lat, dvar_hnod, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    lb = ax.contour(lon, lat, dvar_hnod, cflevels, colors='grey', alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    if conf['trackon']=='yes':
        ...
        if nhour <= len(fhour):
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                mnmx="(min,max)="+"(%6.1f"%np.nanmin(dvar_hnod)+","+"%6.1f)"%np.nanmax(dvar_hnod)
                plt.text(lon_adeck[nhour]-2.15-central_longitude,lat_adeck[nhour]-4.75,mnmx,fontsize=8,color='DarkOliveGreen',fontweight='bold',bbox=dict(boxstyle="round",color='w',alpha=0.5))
                ax.set_extent([lon_adeck[nhour]-5.5,lon_adeck[nhour]+5.5,lat_adeck[nhour]-5,lat_adeck[nhour]+5],crs=ccrs.PlateCarree())
            else:
                print('Longitude track limits are out of the ocean domain')
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature Change ($^oC$) HNOD'
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    cflevels = np.linspace(19, 32, 27)
    cmap = plt.get_cmap('turbo')
    cf = ax.contourf(lon, lat, var_hoda, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.contour(lon, lat, var_hoda, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    if conf['trackon']=='yes':
        ...
        if nhour <= len(fhour):
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                mnmx="(min,max)="+"(%6.1f"%np.nanmin(var_hoda)+","+"%6.1f)"%np.nanmax(var_hoda)
                plt.text(lon_adeck[nhour]-2.15-central_longitude,lat_adeck[nhour]-4.75,mnmx,fontsize=8,color='DarkOliveGreen',fontweight='bold',bbox=dict(boxstyle="round",color='w',alpha=0.5))
                ax.set_extent([lon_adeck[nhour]-5.5,lon_adeck[nhour]+5.5,lat_adeck[nhour]-5,lat_adeck[nhour]+5],crs=ccrs.PlateCarree())
            else:
                print('Longitude track limits are out of the ocean domain')
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature ($^oC$) HODA'
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    cflevels = np.linspace(-4, 4, 17)
    cmap = plt.get_cmap('RdBu_r')
    cf = ax.contourf(lon, lat, dvar_hoda, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    lb = ax.contour(lon, lat, dvar_hoda, cflevels, colors='grey', alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    if conf['trackon']=='yes':
        ...
        if nhour <= len(fhour):
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                mnmx="(min,max)="+"(%6.1f"%np.nanmin(dvar_hoda)+","+"%6.1f)"%np.nanmax(dvar_hoda)
                plt.text(lon_adeck[nhour]-2.15-central_longitude,lat_adeck[nhour]-4.75,mnmx,fontsize=8,color='DarkOliveGreen',fontweight='bold',bbox=dict(boxstyle="round",color='w',alpha=0.5))
                ax.set_extent([lon_adeck[nhour]-5.5,lon_adeck[nhour]+5.5,lat_adeck[nhour]-5,lat_adeck[nhour]+5],crs=ccrs.PlateCarree())
            else:
                print('Longitude track limits are out of the ocean domain')
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature Change ($^oC$) HODA'
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    cflevels = np.linspace(-4, 4, 17)
    cmap = plt.get_cmap('RdBu_r')
    cf = ax.contourf(lon, lat, var_hoda_hnod, levels=cflevels, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.contour(lon, lat, var_hoda_hnod, cflevels, colors='grey',alpha=0.5, linewidths=0.5, transform=ccrs.PlateCarree())
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=20, shrink=0.6, extendrect=True, ticks=cflevels[::4])
    cb.ax.tick_params(labelsize=8)
    ...
    if conf['trackon']=='yes':
        ...
        if nhour <= len(fhour):
            ...
            if np.logical_and(lon_adeck[nhour]-5.5 > lonmin_new,lon_adeck[nhour]+5.5 < lonmax_new):
                mnmx="(min,max)="+"(%6.1f"%np.nanmin(var_hoda_hnod)+","+"%6.1f)"%np.nanmax(var_hoda_hnod)
                plt.text(lon_adeck[nhour]-2.15-central_longitude,lat_adeck[nhour]-4.75,mnmx,fontsize=8,color='DarkOliveGreen',fontweight='bold',bbox=dict(boxstyle="round",color='w',alpha=0.5))
                ax.set_extent([lon_adeck[nhour]-5.5,lon_adeck[nhour]+5.5,lat_adeck[nhour]-5,lat_adeck[nhour]+5],crs=ccrs.PlateCarree())
            else:
                print('Longitude track limits are out of the ocean domain')
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Sea Surface Temperature ($^oC$) HODA-HNOD'
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
