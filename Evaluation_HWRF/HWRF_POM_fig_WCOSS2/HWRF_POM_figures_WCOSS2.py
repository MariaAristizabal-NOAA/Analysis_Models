...
for n,file in enumerate(files_hwrf_pom):
    ...
    fig,ax = plt.subplots()
    ctr = ax.contourf(lonn,latt,sst,cmap='Spectral_r',**kw,extend='both')
    cbar = fig.colorbar(ctr,extendrect=True)
    cbar.set_label('$^oC$',fontsize=14)
    cs = ax.contour(lonn,latt,sst,[26,27,28,29,30],colors='k',alpha=0.3)
    ...
    fig,ax = plt.subplots()
    ctr = ax.contourf(lonn,latt,sst-sst_f0,cmap='seismic',**kw,extend='both')
    cbar = fig.colorbar(ctr,extendrect=True)
    cbar.set_label('$^oC$',fontsize=14)
