...
temp0 = np.empty((len(zlev_pom),len(oklon)))
temp0[:] = np.nan
for i in np.arange(len(oklon)):
    temp0[:,i] = np.asarray(MODEL0['t'][0,:,oklat[i],oklon[i]])
temp0[temp0 == 0] = np.nan

#depth_pom = zmatrix_pom[:,oklat,:][:,:,oklon][:,:,0]
depth_pom = zmatrix_pom[:,oklat,oklon]
latt_pom =  np.tile(lat_pom[oklat,0],(depth_pom.shape[0],1))

temp = np.empty((len(zlev_pom),len(oklon)))
temp[:] = np.nan
for n,file in enumerate(files_hwrf_pom[-2:]):
    ...
    for i in np.arange(len(oklon)):
        temp[:,i] = np.asarray(MODEL['t'][0,:,oklat[i],oklon[i]])
    temp[temp == 0] = np.nan
